    ...
    inquirer
        .prompt(
            [
                {
                    ...
                    choices: ['View Sales by Department', 'Create Department', 'Exit']
            ...
            switch (answer.menu) {
                ...
                case 'View Sales by Department': viewProductsByDept();
                    break;
                case 'Create Department': createDepartment();
                    break;
...
function viewProductsByDept() {
    ...
    connection.query(
        'SET SESSION sql_mode="STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION";SELECT d.department_id AS dept_id, d.department_name AS dept_name, IFNULL(d.over_head_costs,0) AS overhead_cost, p.department_name, IFNULL(SUM(p.product_sales), 0) AS product_sales, IFNULL(SUM(p.product_sales), 0) - IFNULL(d.over_head_costs, 0) AS total_profit FROM departments AS d LEFT JOIN products AS p ON d.department_name = p.department_name GROUP BY d.department_name ORDER BY total_profit',
        function (err, res) {
            if (err) throw err;
            ...
            function buildHeaders(arr) {
                ...
                console.log('\n                                                           Total Profit by Department\n'.green.underline.bgBlack +
                    headers.bold.bgBlack + '\n' +
                    '----------------| ---------------| ---------------| ---------------| ---------------|'.grey)
            }
                    ...
                    [
                        ...
                        var query = connection.query('INSERT INTO departments SET ?',
                            ...
                            function (err, res) {
                                ...
                                console.log('\n\n' + answer.department_name.green.bold + ' added to the department list...' +
